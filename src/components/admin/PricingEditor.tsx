...
export default function PricingEditor({ initial = [], unit = "ip", onChange }: PricingEditorProps) {
  ...
  function validatePriceRows(candidate: ProductPrice[]) {
    ...
    for (const r of candidate) {
      if (r.number_of_ips == null || Number.isNaN(Number(r.number_of_ips)))
        errs.push(`Invalid quantity: ${r.number_of_ips}`);
      if (r.price == null || Number.isNaN(Number(r.price)))
        errs.push(`Invalid price for ${r.number_of_ips}`);
  ...
  function applyParsed(
    parsed: { quantity: number; unit?: string; price: number; currency?: string }[],
  ) {
    const newRows = parsed.map((p, i) => ({
      id: Date.now() + i,
      number_of_ips: p.quantity,
      price: p.price,
    ...
    if (generateMode === "replace") next = newRows;
      ...
      next = Array.from(map.values()).sort(
        (a, b) => Number(a.number_of_ips) - Number(b.number_of_ips),
      );
    }
  ...
  function addRow() {
    const next = [
      ...rows,
      { id: Date.now(), number_of_ips: 1, price: 0, currency: "GHS", sort_order: rows.length },
    ];
    setRows(next);
    onChange?.(next);
  }
      ...
      <div>
        <div className="mb-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button type="button" className="text-sm text-muted-foreground" onClick={addRow}>
              Add row
            </button>
            <button
              type="button"
            ...
            <button
              ...
              onClick={() => {
                // export CSV
                const csv = rows
                  .map((r) => `${r.number_of_ips},${r.price},${r.currency}`)
                  .join("\n");
                    ...
                    if (Array.isArray(parsed)) {
                      applyParsed(
                        parsed.map((p: any, i: number) => ({
                          quantity: Number(p.number_of_ips || p.quantity),
        ...
        <div className="space-y-2">
          {rows.map((r, i) => (
            <div key={r.id} className="flex gap-2 items-center">
              <input
                type="number"
                value={r.number_of_ips}
                onChange={(e) => handleRowChange(i, { number_of_ips: Number(e.target.value) })}
              ...
              <input
                type="number"
                value={r.price}
                onChange={(e) => handleRowChange(i, { price: Number(e.target.value) })}
          ...
          {rows.length === 0 ? (
            ...
            rows.map((r) => (
              <div key={r.id} className="flex justify-between text-sm">
                <span>
                  {r.number_of_ips} {r.number_of_ips === 1 ? unitLabel : quantityLabel}
                </span>
                <span>
                  {r.currency} {r.price}
                </span>
