...
function parseDataOrderMetadata(order: AdminOrder) {
  ...
  if (order.refill_notes) {
    try {
      parsed =
        typeof order.refill_notes === "string"
          ? JSON.parse(order.refill_notes)
          : order.refill_notes;
    } catch {
      parsed = {};
    }
  ...
  return {
    customerName: String(
      parsed?.full_name || parsed?.customer_name || order.customer_name || "",
    ).trim(),
    ...
    bundleName: String(
      parsed?.bundle || parsed?.bundles?.[0] || order.plan_name || order.product_name || "",
    ).trim(),
    bundleSize: String(parsed?.bundle || order.plan_name || "").trim(),
    volumeInMb: parsed?.volume_in_mb ?? parsed?.volumeInMB ?? null,
    deliveryNumber: String(
      parsed?.delivery_number || parsed?.deliveryNumber || order.refill_password || "",
    ).trim(),
  };
}
...
function AdminOrderDetailPage() {
  ...
  const {
    data: remaStatus,
    isFetching: isRemaFetching,
    refetch: refetchRemaStatus,
  } = useQuery({
    queryKey: ["admin-order-rema-status", orderId],
    queryFn: async () => await apiFetch<RemaStatusInfo>(`/api/admin/orders/${orderId}/rema-status`),
    enabled: Boolean(order && isDataOrder),
    refetchInterval: isDataOrder ? 15000 : false,
    retry: false,
  });
  ...
  const canFetchRema = Boolean(order && isDataOrder);
  ...
  const summaryItems = useMemo(() => {
    ...
    if (isDataOrder) {
      const md = dataMetadata;
      const remaReference = remaStatus?.reference || order.fulfillment_reference || null;
      return [
        ["BrokeFlex Order ID", formatOrderReference(order.order_number)],
        ["Rema Data Reference", remaReference || "Waiting for provider..."],
        ["Rema Order Reference", remaStatus?.reference || "Waiting for provider..."],
        ["Rema Client Reference", remaStatus?.clientReference || "Waiting for provider..."],
        ["Customer Name", md?.customerName || "Waiting for provider..."],
        ["Customer Email", md?.customerEmail || order.customer_email || "Waiting for provider..."],
        ["Customer Phone Number", md?.contactNumber || "Waiting for provider..."],
        ["Network", md?.network || "Waiting for provider..."],
        ["Bundle Size", md?.bundleSize || "Waiting for provider..."],
        ["Amount Paid", formatMoney(order.total_amount, order.currency)],
        ["Delivery Number", md?.deliveryNumber || "Waiting for provider..."],
        ["Payment Status", order.payment_status === "paid" ? "Paid" : "Unpaid"],
        ["Order Status", remaStatus?.status || ORDER_STATUS_LABEL[order.status as OrderStatus]],
        ["Delivery Status", order.delivery_status || "pending"],
        ["Payment Reference", order.payment_reference || "Waiting for provider..."],
  ...
  }, [order, isDataOrder, dataMetadata, remaStatus]);
    ...
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-12 sm:px-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <Badge variant="outline" className={statusTone(status)}>
          {isDataOrder && remaStatus?.status ? remaStatus.status : ORDER_STATUS_LABEL[status]}
        </Badge>
      </div>
      ...
      {isDataOrder ? (
        <div className="rounded-2xl border border-primary/30 bg-primary/10 px-4 py-3 text-sm">
          <p className="font-medium text-primary">
            This order is managed automatically by the Rema Data API.
          </p>
          <p className="mt-0.5 text-muted-foreground">
            Status updates cannot be changed manually. The order status and delivery status shown
            below come directly from Rema Data.
          </p>
        </div>
      ) : null}
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Full metadata and fulfillment information for this order.
                  </p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Order progress and fulfillment events.
                  </p>
                </div>
            ...
            <Card className="border-border/70">
              <CardContent className="p-6">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Uploaded evidence for this delivery.
                </p>
          ...
          <Card className="border-border/70">
            <CardContent className="space-y-4 p-6">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Review the contact fields and order metadata.
                </p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                  ...
                  <p className="mt-2 font-medium">
                    {isDataOrder
                      ? displayValue(dataMetadata?.customerEmail)
                ...
                <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                  ...
                  <p className="mt-2 font-medium">
                    {isDataOrder
                      ? displayValue(dataMetadata?.customerName)
                      : (order.customer_name ?? "—")}
                  </p>
                </div>
                <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                  ...
                  <p className="mt-2 font-medium">
                    {isDataOrder
                      ? displayValue(dataMetadata?.contactNumber)
                      : (order.refill_email ?? "—")}
                  </p>
                </div>
                <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                  ...
                  <p className="mt-2 font-medium">
                    {isDataOrder
                      ? displayValue(dataMetadata?.deliveryNumber)
            ...
            <Card className="border-border/70">
              <CardContent className="space-y-4 p-6">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Bundle-level metadata for this order.
                  </p>
                </div>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
              ...
              <CardContent className="p-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      Update status, add CD key/refill proof, or record a note for support.
                    </p>
                  </div>
                  ...
                  {isProxyOrder ? (
                    <div className="space-y-4">
                      <div>
                        ...
                        />
                        <p className="mt-2 text-sm text-muted-foreground">
                          Adding a CD key here will ensure the customer can receive it by email if
              ...
              <CardContent className="p-6 space-y-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button
                    size="sm"
                    variant="secondary"
                    onClick={() => void refetchRemaStatus()}
                    disabled={!canFetchRema || isRemaFetching}
                  >
                ...
                {order.fulfillment_reference || remaStatus?.reference ? (
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                      ...
                      <p className="mt-2 font-medium">
                        {displayValue(order.fulfillment_reference || remaStatus?.reference)}
                      </p>
                    </div>
                    <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                    ...
                    <p className="mt-2 font-medium">
                      {displayValue(remaStatus?.providerReference)}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Chronological events for this order.
                  </p>
                </div>
                <Button size="sm" variant="secondary" onClick={() => void refetchEvents()}>
                  Refresh
                    ...
                    >
                      <div className="flex items-center justify-between gap-4">
                        <p className="font-medium">
                          {ORDER_STATUS_LABEL[event.status as OrderStatus] ?? event.status}
                        </p>
