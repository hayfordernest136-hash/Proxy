...
export const Route = createFileRoute("/data/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse live data bundles from MTN, Telecel, and AirtelTigo.",
      },
      { property: "og:title", content: "Data Store - BrokeFlex" },
      {
        property: "og:description",
        content: "Secure data bundle checkout with instant Paystack payments.",
      },
    ],
  }),
...
function DataIndexPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const data = await apiFetch<DataBundle[]>(
        `/api/data/bundles?network=${encodeURIComponent(selectedNetwork)}`,
      );
      return data;
    },
    staleTime: 60_000,
            ...
            <div className="rounded-2xl border border-border/70 bg-muted/25 p-4">
              ...
              <div className="mt-4 space-y-3">
                ...
                <Button
                  ...
                  variant="secondary"
                  onClick={addToCart}
                  disabled={!selectedBundle || isAddingToCart}
                >
                  {isAddingToCart ? (
              ...
              {user ? (
                <p className="mt-4 text-xs text-muted-foreground">
                  Logged-in customers will also see this order in order history.
                </p>
              ) : (
                <p className="mt-4 text-xs text-muted-foreground">
                  No account required. Guest purchases complete using the details entered during
                  checkout.
                </p>
              )}
            </div>
