...
export async function register(req: Request, res: Response) {
  if (!isDatabaseAvailable) {
    return res.status(503).json({
      message: "Authentication is temporarily unavailable because the database is unreachable.",
    });
  }
  ...
  try {
    ...
    if (
      ...
      if (!current_password || !new_password || !confirm_password) {
        return res
          .status(400)
          .json({ message: "Please fill in the current password, new password, and confirmation" });
      }
