...
import { createOrder, createOrderEvent, getOrderById, updateOrder, type OrderRow } from '../services/order.service';
...
function normalizeRemaNetwork(value: string | undefined, fallbackNetwork: string) {
  const normalized = String(value || fallbackNetwork || 'mtn').trim().toLowerCase();
...
function normalizeBundle(item: any, fallbackNetwork: string, index: number) {
  const rawName = String(item?.name ?? item?.bundle_name ?? item?.title ?? item?.label ?? item?.volume ?? item?.size ?? '').trim();
  ...
  const rawPrice = Number(item?.price ?? item?.amount ?? item?.api_price ?? item?.bundle_price ?? item?.cost);
  ...
  const id = String(item?.id ?? item?.bundle_id ?? item?.reference ?? item?.product_id ?? `${network}-${volume || name || index + 1}`).trim();
  ...
  const price = Number.isFinite(markupPercent) && markupPercent > 0
...
function extractPayload(payload: any) {
  ...
  if (payload && typeof payload === 'object') {
    return payload.data ?? payload.result ?? payload.response ?? payload.bundles ?? payload.plans ?? payload.products ?? payload.packages ?? payload.items ?? payload.results ?? null;
  }
  return null;
}
...
async function fetchLiveRemaBundlesForNetwork(networkValue: string): Promise<any[]> {
  ...
  const bundlesUrl = String(process.env.REMA_API_BUNDLES_URL || process.env.REMA_API_LIST_URL || '').trim();
  const baseEndpoint = bundlesUrl || `${baseUrl.replace(/\/+$/, '')}${String(process.env.REMA_API_BUNDLES_PATH || '/bundles').trim()}`;
...
function buildGuestCustomerMetadata(customer: any, items: any[]) {
  ...
  const firstDelivery = String(firstItem?.deliveryNumber || firstItem?.delivery_number || '').trim();
  ...
  return JSON.stringify({
    ...
    delivery_numbers: (items || []).map((item: any) => item.deliveryNumber || item.delivery_number || ''),
...
function parseGuestMetadata(order: OrderRow) {
  ...
  try {
    const parsed = typeof order.refill_notes === 'string' ? JSON.parse(order.refill_notes) : order.refill_notes;
    return {
      contact_number: String(parsed?.contact_number || parsed?.customer_contact_number || '').trim(),
      ...
      delivery_numbers: Array.isArray(parsed?.delivery_numbers)
        ? parsed.delivery_numbers.map((value: unknown) => String(value || '').trim()).filter(Boolean)
        : [],
...
async function fetchRemaOrderStatus(reference: string) {
  ...
  try {
    ...
    if (!response.ok) {
      if (isDevelopmentMode()) {
        console.error('[Data API] Rema order-status failed', { status: response.status, body: payload ?? responseText });
      }
      return null;
    }
...
export async function createDataOrderHandler(req: Request, res: Response) {
  try {
    ...
    const deliveryNumbers = items.map((item: any) => String(item.deliveryNumber || item.delivery_number || '').trim());
    ...
    await createOrderEvent(order.id, 'awaiting_payment', 'Data order created. Waiting for payment.');
...
export async function trackDataOrderHandler(req: Request, res: Response) {
  try {
    ...
    const searchContactNumber = normalizeTrackingNumber(String(req.query.contactNumber || '').trim());

    // Require BOTH the order ID and the contact number to track an order.
    if (!searchOrderId || !searchContactNumber) {
      return res.status(400).json({ message: 'Provide both the order ID and contact number to track your order.' });
    }
    ...
    const [rows] = await pool.query('SELECT * FROM orders WHERE order_number = ? LIMIT 1', [parsedOrderNumber]);
    ...
    if (!contactsMatch(storedNumbers, searchContactNumber)) {
      return res.status(404).json({ message: 'No matching data order was found for this order ID and contact number.' });
    }

    let remaStatusInfo: { status: string; reference: string; message: string } | null = null;
    if (order.fulfillment_reference) {
      remaStatusInfo = await fetchRemaOrderStatus(order.fulfillment_reference);
    }
    ...
    if (remaStatusInfo?.status) {
      ...
      if (localStatus === 'delivered' && order.status !== 'completed') {
        ...
        await createOrderEvent(order.id, 'completed', `Rema reported delivery: ${remaStatusInfo.message || remaStatusInfo.status}`);
      } else if (localStatus === 'failed' && order.status !== 'failed') {
        await updateOrder(order.id, { status: 'failed', delivery_status: 'failed' });
        await createOrderEvent(order.id, 'failed', `Rema reported failure: ${remaStatusInfo.message || remaStatusInfo.status}`);
      } else if (localStatus === 'processing' && order.payment_status === 'paid' && order.status !== 'processing') {
    ...
    const fulfillmentReference = remaStatusInfo?.reference || order.fulfillment_reference || undefined;
...
export async function listDataBundlesHandler(req: Request, res: Response) {
  ...
  const baseUrl = process.env.REMA_API_BASE_URL || process.env.REMA_BASE_URL || 'https://remadata.com/api';
  const apiKey = process.env.REMA_API_KEY || process.env.REMA_API_TOKEN || '';

  const normalizedNetwork = network.toLowerCase().replace(/\s+/g, '');

  const networkValue = normalizedNetwork === 'airteltigo' || normalizedNetwork === 'airtel' || normalizedNetwork === 'airtel-tigo'
    ? 'airteltigo'
    : normalizedNetwork === 'telecel' || normalizedNetwork === 'telex' || normalizedNetwork === 'tigo'
      ? 'telecel'
  ...
  try {
    ...
    const bundlesUrl = String(process.env.REMA_API_BUNDLES_URL || process.env.REMA_API_LIST_URL || '').trim();
    const endpoint = bundlesUrl || `${baseUrl.replace(/\/+$/, '')}${String(process.env.REMA_API_BUNDLES_PATH || '/bundles').trim()}`;
    ...
    if (isDevelopmentMode()) {
      console.log('[Data API] returning normalized bundles', { count: pricedBundles.length, network: networkValue });
    }
