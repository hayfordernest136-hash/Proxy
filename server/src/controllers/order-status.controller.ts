...
import { getOrderByFulfillmentReference, updateOrder, getOrderById, createOrderEvent } from '../services/order.service';
import { getRemaApiBaseUrl, getRemaApiHeaderName, getRemaApiKey, logRemaDebug, remaStatusToLocalStatus } from './admin.controller';
...
export async function getOrderStatusHandler(req: Request, res: Response) {
  try {
    ...
    if (localStatus && localStatus !== order.status) {
      ...
      await createOrderEvent(order.id, localStatus, `Live status sync from Rema: ${statusInfo.status}`);
    }
...
async function fetchRemaOrderStatus(reference: string) {
  ...
  try {
    ...
    return {
      ...
      providerReference: String(data?.provider_reference ?? payload?.provider_reference ?? '').trim(),
