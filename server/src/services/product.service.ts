...
export async function createProduct(product: {
  ...
  // ensure uniqueness
  const [existsRows] = await pool.query("SELECT COUNT(*) as count FROM products WHERE slug = ?", [
    slug,
  ]);
  ...
  try {
    ...
    if ((product as any).prices && Array.isArray((product as any).prices)) {
      ...
      for (const raw of prices) {
        ...
        await conn.query(
          "INSERT INTO product_prices (product_id, number_of_ips, price, currency, sort_order) VALUES (?, ?, ?, ?, ?)",
          [insertId, p.number_of_ips, p.price, p.currency, sort],
        );
        ...
        await conn.query(
          ...
          [insertId, `${p.number_of_ips} IPs`, p.price, p.currency, 1, sort, p.number_of_ips],
        );
        sort += 1;
      }
    }
    ...
    try {
      ...
      console.warn("Failed to rollback product creation", error);
    }
    throw err;
  } finally {
    conn.release();
  ...
  try {
    ...
    if (patch.prices && Array.isArray(patch.prices)) {
      ...
      for (const raw of patch.prices) {
        ...
        await conn.query(
          "INSERT INTO product_prices (product_id, number_of_ips, price, currency, sort_order) VALUES (?, ?, ?, ?, ?)",
          [productId, p.number_of_ips, p.price, p.currency, sort],
        );
        await conn.query(
          "INSERT INTO plans (product_id, name, price, currency, is_active, sort_order, number_of_ips) VALUES (?, ?, ?, ?, ?, ?, ?)",
          [productId, `${p.number_of_ips} IPs`, p.price, p.currency, 1, sort, p.number_of_ips],
        );
        sort += 1;
      }
    }
    ...
    try {
      ...
      console.warn("Failed to rollback product update", error);
    }
    throw err;
  } finally {
    conn.release();
...
export async function softDeleteProduct(productId: number) {
  await pool.query("UPDATE products SET is_active = 0, updated_at = NOW() WHERE id = ?", [
    productId,
  ]);
  return true;
}
