...
export async function getNextOrderNumber(connection?: any) {
  ...
  const [rows] = connection ? await connection.query(query) : await pool.query(query);
...
export async function createOrder(order: {
  ...
  try {
    ...
    const orderType =
      order.order_type?.trim() || (order.delivery_method === "data_bundle" ? "data" : "proxy");
...
export async function setOrderPaymentFee(
  orderId: number,
  paymentFee: number,
...
export async function getOrderByFulfillmentReference(reference: string) {
  const [rows] = await pool.query("SELECT * FROM orders WHERE fulfillment_reference = ? LIMIT 1", [
    reference,
  ]);
  return (rows as OrderRow[])[0] ?? null;
}
